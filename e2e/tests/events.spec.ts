...
test.describe.serial(`${name} - Events`, () => {
  ...
  test("page_view event is tracked", async ({ page, request }) => {
    const response = await getPayload(page, request);
    const event = response.data.events.find((e: any) => e.type === "page_view");
      ...
      .poll(
        async () => {
          ...
          return data.data?.events?.find(
            (e: any) => e.type === "custom" && e.data.name === "outbound_click",
          );
        },
        { timeout: 10000 },
      )
    ...
    await expect
      .poll(
        async () => {
          ...
          if (!data) return null;
          return data.data?.events?.find(
            (e: any) => e.type === "custom" && e.data.name === "file_download",
          );
        },
        { timeout: 10000 },
      )
  ...
  test("unhandled_rejection is tracked", async ({ page, request }) => {
    ...
    const event = response.data.events.find(
      (e: any) => e.type === "custom" && e.data.name === "unhandled_rejection",
    );
    expect(event).toBeDefined();
  });
