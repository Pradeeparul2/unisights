/**
 * tests/adapter.node.test.ts
 * Unit tests for the Node.js middleware adapter (Express, NestJS, raw http).
 */
...
function makeAdapter(handler = null as any, path = "/events") {
...
// ── Raw Node http response (no Express .status/.json) ─────────────────────────
...
describe("nodeAdapter — raw Node ServerResponse", () => {
  it("writes correct status and JSON via writeHead/end", async () => {
    ...
    const req = makeMockReq({ body: {} });
    const res = makeMockRes();
    // Remove express-style methods to force raw path
