/**
 * tests/adapter.koa.test.ts
 * Unit tests for the Koa adapter.
 */
...
function makeMiddleware(handler = null as any, path = "/events") {
...
describe("koaAdapter — always 200", () => {
  ...
  it("returns 200 when raw stream body is invalid JSON", async () => {
    const middleware = makeMiddleware();
    const ctx = makeMockKoaCtx({ path: "/events" }); // no requestBody → uses raw stream
    const promise = middleware(ctx as any, noop);
    process.nextTick(() => {
...
describe("koaAdapter — body parsing", () => {
  ...
  it("falls back to raw stream when ctx.request.body is undefined", async () => {
    const handler = vi.fn();
    const middleware = makeMiddleware(handler);
    const ctx = makeMockKoaCtx({ path: "/events" }); // no requestBody
